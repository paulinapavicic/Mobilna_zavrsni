...
import { pick, keepLocalCopy, DocumentResult } from '@react-native-documents/picker';
  ...
  const handlePickFile = async () => {
    try {
      ...
      const [res] = await pick({
        ...
        copyTo: 'documentDirectory', // copies the file to documentDirectory
        types: ['*/*'],
      });

      // Optionally keep a local copy (useful if you want to keep file accessible)
      ...
      fileData.append('materialId', materialId);
      ...
      setUploading(true);
      await uploadEducationalFile(fileData);
      ...
      if (err?.message === 'User cancelled document picker') {
        ...
        Alert.alert('Upload failed', err?.message || 'Try a different file.');
      }
    } finally {
      setUploading(false);
    }
